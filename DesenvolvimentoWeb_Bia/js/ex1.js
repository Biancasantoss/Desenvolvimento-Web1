...
function validar(){
    ...
    //(?=.*?[A-Z]) - minímo uma letra maiúscula
    //(?=.*?[a-z]) - minímo uma letra minúscula
    //(?=.*?[0-9]) - minímo um número
    //(?=.*?[^\w\s]) - minímo um caractere especial
    //.{6,10} - minímo 6, máximo 10 caracteres

     if(!regex.exec(senha)){

        alert("A senha deve conter ao menos uma letra maiúscula, uma letra minúscula, números e caracteres especiais, minímo 6 e máximo de 10 caracteres")
